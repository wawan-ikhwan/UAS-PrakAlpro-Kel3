...
def sortBy(dataframe,kolom,urutan='naik',simpan=False):
  ...
  if urutan != 'naik': sortedList=sortedList[::-1]
  result['g'] = result.groupby(kolom).cumcount()
  df2 = pd.DataFrame({kolom: sortedList})
  df2['g'] = df2.groupby(kolom).cumcount()
  result=result.set_index([kolom, 'g']).reindex(df2.set_index([kolom,'g']).index).reset_index().drop('g',axis=1)
  if simpan:
    result.to_csv(DIR+'/./data/output/sorted/'+kolom+'.csv')
  return result

def searchBy(dataframe,kolom,cari):
  return LinearSearch(dataframe[kolom],cari) # Mengembalikan indeks
#=========================================
sortedDF=sortBy(df,'sales',urutan='turun',simpan=True)
print(sortedDF)
